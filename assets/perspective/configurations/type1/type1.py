...
\begin{tikzpicture}
    \coordinate (a) at (2, 1);
    \coordinate (b) at (3, 2);
    \draw[fill, blue] (a) circle (2pt) node[anchor=east] {$x$} -- (b) circle (2pt) node[anchor=west] {$y$};

    \coordinate (c) at (3, 2.5);
    \coordinate (d) at (4, 1.5);
    \draw[fill, blue] (c) circle (2pt) node[anchor=east] {$z$} -- (d) circle (2pt) node[anchor=west] {$w$};
\end{tikzpicture}
\end{document}'''
...
for conf in ['abcd', 'bacd', 'abdc', 'badc']:
    process(conf)
